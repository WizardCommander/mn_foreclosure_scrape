...
class MullvadManager:
    def __init__(self, enabled=True, auto_connect=True):
        ...
        env_enabled = os.getenv("MULLVAD_ENABLED", "true").lower()
        self.enabled = enabled and env_enabled == "true"
    ...
    def setup(self, retries=3):
        ...
        for attempt in range(retries):
            try:
                logger.info(
                    f"🌐 Setting up Mullvad VPN connection... (attempt {attempt + 1}/{retries})"
                )

                # Check if Mullvad is installed
                result = subprocess.run(
                    ["mullvad", "--help"], capture_output=True, text=True, timeout=10
                )
                if result.returncode != 0:
                    logger.error(
                        "❌ Mullvad CLI not found. Please install Mullvad VPN."
                    )
                    self.enabled = False
                    return False
                ...
                if self.rotate_server():
                    ...
                    if attempt < retries - 1:
                        logger.warning(
                            f"⚠️ VPN connection failed, retrying in 10 seconds... (attempt {attempt + 1}/{retries})"
                        )
                        time.sleep(10)
                    else:
                        logger.error(
                            "❌ Failed to establish VPN connection after all retries"
                        )
                        self.enabled = False
                        return False
                ...
                if attempt < retries - 1:
                    logger.warning(
                        f"⚠️ Mullvad CLI timeout, retrying in 10 seconds... (attempt {attempt + 1}/{retries})"
                    )
                    time.sleep(10)
                else:
                    logger.error(
                        "❌ Mullvad CLI timeout after all retries - please check installation"
                    )
                    self.enabled = False
                    return False
                ...
                if attempt < retries - 1:
                    logger.warning(
                        f"⚠️ VPN setup failed: {e}, retrying in 10 seconds... (attempt {attempt + 1}/{retries})"
                    )
                    time.sleep(10)
                else:
    ...
    def _try_server_commands(self, selected_server):
        """Try multiple command variations for Mullvad CLI compatibility"""
        command_variants = [
            ["mullvad", "relay", "set", "hostname", selected_server],  # Current/newer
            ["mullvad", "relay", "set", "location", selected_server],  # Alternative
            ["mullvad", "relay", "set", selected_server],  # Simplified
            ["mullvad", "relay", "set", "location", "us"],  # Country only fallback
        ]

        for i, cmd in enumerate(command_variants):
            try:
                logger.info(
                    f"🎯 Trying relay command {i+1}/{len(command_variants)}: {' '.join(cmd)}"
                )
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)

                if result.returncode == 0:
                    logger.info(f"✅ Successfully set relay using: {' '.join(cmd)}")
                    return True
                else:
                    logger.warning(f"⚠️ Command {i+1} failed: {result.stderr.strip()}")

            except subprocess.TimeoutExpired:
                logger.warning(f"⚠️ Command {i+1} timed out")
            except Exception as e:
                logger.warning(f"⚠️ Command {i+1} error: {e}")

        logger.error("❌ All relay set commands failed - will try basic connection")
        return False
    ...
    def rotate_server(self, max_wait_time=30):
        ...
        try:
            ...
            disconnect_result = subprocess.run(
                ["mullvad", "disconnect"], capture_output=True, text=True, timeout=15
            )
            time.sleep(3)  # Longer wait to ensure clean disconnect

            # Set relay to specific server with fallback compatibility
            logger.info(f"🎯 Setting relay to {selected_server}...")
            if not self._try_server_commands(selected_server):
                logger.warning(
                    f"⚠️ Could not set specific server {selected_server}, will try basic connection"
                )

            # Connect with longer timeout
            logger.info(f"🔗 Connecting to {selected_server}...")
            connect_result = subprocess.run(
                ["mullvad", "connect"], capture_output=True, text=True, timeout=20
            )
            ...
            logger.info(
                f"⏳ Waiting for VPN connection to stabilize (up to {max_wait_time} seconds)..."
            )
            for i in range(max_wait_time):
                ...
                if self.verify_connection(silent=True):
                    ...
                    self.connection_established = True
                    logger.info(
                        f"✅ Successfully connected to {selected_server} (took {i+1} seconds)"
                    )
                    return True
                if (i + 1) % 5 == 0:  # Progress update every 5 seconds
                    logger.info(
                        f"🔄 Still connecting... ({i+1}/{max_wait_time} seconds)"
                    )

            logger.error(
                f"❌ VPN connection verification failed for {selected_server} after {max_wait_time} seconds"
            )
            return False
    ...
    def verify_connection(self, silent=False):
        ...
        try:
            ...
            result = subprocess.run(
                ["mullvad", "status"], capture_output=True, text=True, timeout=10
            )
    ...
    def ensure_connected(self, retries=2):
        ...
        if not self.connection_established:
            logger.info(
                "🌐 No active VPN connection found, establishing new connection..."
            )
